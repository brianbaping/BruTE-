...
void ConfigFile::ImportDrumMaps()
{
    ...
    for (int i = 0; i < 999; i++)
    {
        std::string drumfileName = "drum-maps\\drum" + std::to_string(i) + ".drummap.txt";

        std::cout << "\tReading " << drumfileName << std::endl;
        ...
        drumfile.open(drumfileName);
        while (std::getline(drumfile, thisline))
        {
            m_log << thisline << std::endl;
...
void ConfigFile::ImportPitchBendInstructions()
{
    ...
    pitchbendfile.close();
    if (m_dohumanization > 0)
        m_log << "Artificial Humanization selected .. results may not be pleasing!" << std::endl;
}
...
void ConfigFile::ParseConfigMapping(std::stringstream *mappingtext)
{
    ...
    m_generalpitch = 0;
    if (myline.size() > 1)
        m_generalpitch = atoi(myline[1].c_str());
    ...
    m_InvertMajor = 0;
    if (myline.size() > 4)
        m_InvertMajor = atoi(myline[4].c_str());
    ...
    m_globalvolume = 0;
    if (myline.size() > 1)
        m_globalvolume = atoi(myline[1].c_str());
    ...
    if (myline.size() > 1)
        m_volumecompress = atof(myline[1].c_str());

    m_nopitchguessing = false;
    std::getline(myconfigfile, mappingtextline);
    myline = split(mappingtextline, ' ');
    if (myline[0].c_str()[0] == 'n')
        m_nopitchguessing = true;

    m_nobackfolding = false;
    std::getline(myconfigfile, mappingtextline);
    myline = split(mappingtextline, ' ');
    if (myline[0].c_str()[0] == 'n')
        m_nobackfolding = true;

    m_fadeoutlength = 0;
    std::getline(myconfigfile, mappingtextline);
    myline = split(mappingtextline, ' ');
    if (myline.size() > 2)
        m_fadeoutlength = atoi(myline[2].c_str());
            ...
            if (thisline[0].size() > 0)
                if (thisline[0][0] != '%')
                {
                    ...
                    if (thisline[0] == "polyphony")
                    {
                        ...
                        if (thisline[2] == "top")
                            m_mpolyd = 1;
                        if (thisline[2] == "bottom")
                            m_mpolyd = 0;
                    }
                    if (thisline[0] == "instrument") // instrument <name> <drumstyle> <c/insample> <outsample?>
                    {
                        for (int j = 0; j < nInstruments; j++)
                            if (thisline[1] == lotroinstruments[j])
                                m_minstrument = j;
                        for (int j = 0; j < nInstruments; j++)
                            if (thisline[1] == lotroinstruments2[j])
                                m_minstrument = j;
                        for (int j = 0; j < nInstruments; j++)
                            if (thisline[1] == lotroinstruments3[j])
                                m_minstrument = j;

                        if (thisline.size() > 2)
                        {
                            if (m_minstrument == 8)
                                m_drumstyle = atoi(thisline[2].c_str());
                        }
                        if (thisline.size() > 3)
                        {
                    ...
                    if (thisline[0] == "duration")
                    {
                        ...
                        if (thisline.size() > 2)
                            m_maxdur = atoi(thisline[2].c_str());
                    }
                    ...
                    if (thisline[0] == "arpeggio")
                    {
                        ...
                        if (m_arpeggioduration == 1)
                            m_arpeggioduration = 2;
                        if (thisline.size() > 2)
                            m_arpeggioinc = atoi(thisline[2].c_str());
                    }
                    ...
                    if (thisline[0] == "miditrack")
                    {
                        ...
                        if (thisline.size() > 6)
                        {
                            ...
                            if (atof(thisline[7].c_str()) > m_DATA_maxdelay)
                                m_DATA_maxdelay = atof(thisline[7].c_str());
                        }
                        else
                        {
...
void ConfigFile::ImportConfigFile(char *infilename)
{
    ...
    std::cout << "Opening Mapping File: " << infilename << std::endl;
    ...
    std::vector<std::string> thisline;
    std::string mappingtextline;
    while (std::getline(myconfigfile, mappingtextline))
    {
        // read next line
        //std::string line = readline(myconfigfile);

        std::cout << "Parsing: " << mappingtextline << std::endl;
